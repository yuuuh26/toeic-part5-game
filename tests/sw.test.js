...
test('an update keeps downloaded music and leaves other apps caches untouched',async()=>{
 ...
 const current=await caches.open('part5-burst-v1.0.6');assert.equal(await (await current.match('assets/bgm/the-winning-move.mp3')).text(),'music');assert.equal(await current.match('src/app.js'),undefined);
});
test('cache quota errors do not turn successful network requests into failures',async()=>{
 ...
 const {handlers,caches}=setup(async()=>response);const current=await caches.open('part5-burst-v1.0.6');current.put=async()=>{throw Error('quota');};
